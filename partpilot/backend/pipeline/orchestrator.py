...
Defines the *future* execution flow only. Every stage call below raises
`NotImplementedError` transitively through its interface implementation
until each Brain is actually built. The orchestrator itself contains no
AI logic — it is pure composition/wiring, following dependency
inversion (it depends on the `*Interface` ABCs, never concrete classes).
"""
